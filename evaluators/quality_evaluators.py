...
class MeanRecallEvaluator(BaseQualityEvaluator):
    ...
    def _preprocess_metric_for_thresholds(self, metric_list, target_values):
        ...
        if self.method == 'pearson':
            ...
            raise ValueError(f"Unknown method: {self.method}. Use 'pearson' or 'spearman'")
        ...
        if not np.isnan(corr) and corr > self.positive_thr:
            ...
            inverted = True
            logger.debug(f"Metric inverted due to positive correlation ({self.method} r={corr:.3f})")
    ...
    def _find_threshold_binary(self, class_list, metric_list):
        ...
        """
        # Preprocess metric
        transformed_metric, inverted, _ = self._preprocess_metric_for_thresholds(metric_list, class_list)
    ...
    def compute_metric(self, similarity_scores: List[float], labels: List[Any]) -> Dict[str, Any]:
        """
        ...
        logger.warning(f"MeanRecallEvaluator supports 2 or 3 classes, got {n_classes}. Using binary approach for each pair.")
        results = {}
        for c1, c2 in itertools.combinations(unique_classes, 2):
            mask = np.isin(y_true, [c1, c2])
            if np.sum(mask) < 2:
