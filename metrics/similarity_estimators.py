        ...
        for f_patch, w_patch, target in tqdm(dataset, desc=self.name):
            try:
                ...
                if not isinstance(f_patch, torch.Tensor):
                    f_patch = (
                        torch.from_numpy(f_patch)
                        if isinstance(f_patch, np.ndarray)
                        else torch.tensor(f_patch)
                    )
                if not isinstance(w_patch, torch.Tensor):
                    w_patch = (
                        torch.from_numpy(w_patch)
                        if isinstance(w_patch, np.ndarray)
                        else torch.tensor(w_patch)
                    )
...
class CellPoseSimilarityEstimator(BaseSimilarityEstimator):
    ...
    def __init__(self, device: str = "cuda", batch_size: int = 16):
        ...
        try:
            self.model = models.CellposeModel(
                gpu=(device == "cuda"), pretrained_model="cyto2_cp3"
            )
            self.name = "CellPoseSAM"
            logger.info(
                f"Initialized CellPose model on {device} with batch_size={batch_size}"
            )
        except Exception as e:
    ...
    def compute_scores(self, dataset: Dataset) -> Dict[str, List[float]]:
                ...
                for batch_idx, batch in enumerate(tqdm(loader, desc=self.name)):
                    try:
                        ...
                        for i, blk in enumerate(encoder.blocks):
                            ...
                            dist_metric = (
                                self.normalized_dist(x0, x1).float().cpu().numpy()
                            )

                            results[f"{self.name}_sim_layer_{i}"].extend(sim.tolist())
                            results[f"{self.name}_dist_layer_{i}"].extend(
                                dist_metric.tolist()
                            )
                        ...
                        results[f"{self.name}_sim_neck"].extend(
                            self.cosine_sim(x0_neck, x1_neck)
                            .float()
                            .cpu()
                            .numpy()
                            .tolist()
                        ...
                        results[f"{self.name}_dist_neck"].extend(
                            self.normalized_dist(x0_neck, x1_neck)
                            .float()
                            .cpu()
                            .numpy()
                            .tolist()
        ...
        logger.info(
            f"Computed {len(results)} metrics for {len(list(results.values())[0])} samples"
        )
        return dict(results)
